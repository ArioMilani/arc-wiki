...
const CACHE_KEY = 'arc_wiki_v20_manual_override'; 

// --- MANUAL OVERRIDES (Based on your Cheat Sheet) ---
// We match these partial strings against the Item Name to force categories.
const OVERRIDES = {
    QUEST: [
        "Adrenaline", "Unique Uplink", "Drone Tool", "Barricade", 
        "Camera Lens", "Antiseptic", "Standard Core", "Syringe", 
        "Surge Splint", "Thumper", "Optics Package", "Data Drive",
        "Memory Chip", "Gyroscope"
    ],
    PROJECT: [
        "Advanced ARC", "Compact Power", "Magnetic Stabilizer", "Cluster Module",
        "Advanced Electrical", "Modulator", "Sensors", "Cooling Fan", 
        "Battery", "Light Bulb", "Electrical Component", "Wire", 
        "Durable Cloth", "Steel Cabling", "ARC Alloy", "Rubber Parts", 
        "Hard Plastic", "Transformer", "Ball Bearing"
    ],
    UPGRADE: [
        // Gunsmith
        "Angled Grip", "Muzzle", "Scope", "Stock", "Receiver", "Barrel", "Magazine",
        // Medical
        "Tweezers", "Blood Bag", "Monitor", "Pills", "Chemicals",
        // Explosives
        "Gas Canister", "Spark Plug", "Fuse", "Bottle", "Gunpowder", "Igniter",
        // Gear / Scrappy / Utility
        "Cable", "Screen", "Lens", "Circuit", "Resin", "Canister", "Filter",
        "Toolbox", "Drill", "Fruit", "Vegetable", "Food", "Water", "Seed",
        "Tape", "Glue", "Fabric", "Metal Sheet", "Anvil"
    ]
};

const checkCategory = (name: string, list: string[]) => {
...
const ArcImage = ({ item, style }: { item: ProcessedItem, style: CSSProperties }) => {
  ...
  useEffect(() => { setSrc(`${BASE_URL}/${item.id}.png`); }, [item.id]);
...
function App() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        // Clear old cache to ensure new logic applies
        localStorage.removeItem(CACHE_KEY);

        // We only fetch the items list now, relying on our hardcoded logic for categorization
        ...
        const processed = rawItems
          ...
          .map((item: any) => {
            ...
            // This forces items into categories based on the Cheat Sheet
            const isQuest = checkCategory(name, OVERRIDES.QUEST);
            // Project check: must match project list AND not be a quest item
            const isProject = !isQuest && checkCategory(name, OVERRIDES.PROJECT);
            // Upgrade check: must match upgrade list AND not be quest/project
            const isUpgrade = !isQuest && !isProject && checkCategory(name, OVERRIDES.UPGRADE);
            
            // Default to recycle if nothing else matches
  ...
  const s: { [key: string]: CSSProperties | ((arg: any) => CSSProperties) } = {
    ...
    header: { 
        backgroundColor: '#0a0a0a', borderBottom: '1px solid #222', padding: '0 40px', height: '80px',
    ...
    raidToggle: (active: boolean) => ({
        background: active ? '#ff9800' : '#333', width: 44, height: 24, borderRadius: 20, position:'relative', cursor:'pointer'
    }),
  ...
  const SectionRow = ({ title, cat, color }: { title: string, cat: CategoryType, color: string }) => {
    ...
    // Show all categories even if empty to verify logic is working
    ...
    return (
      <div style={{marginBottom: 50}}>
        <div style={s.sectionHeader as CSSProperties}>
            ...
            <button 
                style={s.seeAllBtn as CSSProperties}
                onClick={() => goCategory(cat)}
                onMouseEnter={(e: SyntheticEvent<HTMLButtonElement>) => { e.currentTarget.style.borderColor = color; e.currentTarget.style.color = color; }}
                onMouseLeave={(e: SyntheticEvent<HTMLButtonElement>) => { e.currentTarget.style.borderColor = '#333'; e.currentTarget.style.color = '#888'; }}
            >
                SEE ALL →
            </button>
        </div>
        ...
        {data.length === 0 ? (
            <div style={{color:'#444', fontStyle:'italic', paddingLeft:20}}>No items found in this category</div>
        ) : (
